    ...
    <div class="col-12">
      ...
      <div class="box" style="border-top:#1E58C1 4px solid; padding: 20px;">
        <div class="card-header">
          <h3 class="card-title"><b> DATA LINEN PACKING </b></h3>
        ...
        <div class="card-body">
          <table id="example1" class="table table-bordered table-hover">
            ...
            <tbody>
            	<?php $no2=0;
									$query = "SELECT * from tb_packed
									inner join tb_jabatan on tb_packed.id_jabatan = tb_jabatan.id_jabatan
									where id_packed order by id_packed ASC";
									...
									while($row = mysqli_fetch_assoc($result)) 
							{
								...
								?><tr><?php
									...
									?><td align="center"> 
											...
											<button class="btn btn-success" title="Lihat Data" onclick="Lihatlinen('<?=$row['id_packed']?>', '<?=$row['packing_code']?>', '<?=$row['packing_date']?>', '<?=$row['packed_by']?>', '<?=$row['hotel_code']?>', '<?=$row['hotel_name']?>', '<?=$row['total']?>', '<?=$row['status']?>')" ><i class="fa fa-eye" <?php if($public_id_jabatan != 3 || $public_id_jabatan != 5){ print "disabled";}?>></i></button>
									
									<button class="btn btn-danger" title="Hapus Data" onclick="Hapus('<?=$row['id_packed']?>')"><i class="fa fa-trash" <?php if($public_id_jabatan != 3){ print "disabled";}?>></i></button>
...
<div class="modal-dialog modal-md" >
	...
	<div class="modal-content">
	  ...
	  <div class="modal-body row">
		  ...
		  <input type="hidden" id="id_packed" value="">
...
$('#btn_save').on('click', function (e) {
	...
	if(id_packed == '')
	{
		...
		{	
			$.post("func/func_administrator.php",{token:token, id_packed:'', packing_code:packing_code, packing_date:packing_date, packed_by:packed_by, hotel_code:hotel_code, hotel_name:hotel_name, total:total, status:status, kode:'packed'},
			function(data)
			{
				if(data == 1)
				{				
					swal({text: "Data packed berhasil disimpan!", icon: 'success',}).then((result) => {window.location.reload();})
				}
				else
				{
	...
	{
			$.post("func/func_administrator.php",{token:token, id_packed:id_packed, packing_code:packing_code, packing_date:packing_date, packed_by:packed_by, hotel_code:hotel_code, hotel_name:hotel_name, total:total, status:status, kode:'packed'},
		function(data){
			if(data == 1)
			{				
				swal({text: "Data packed berhasil diperbaharui!",icon: 'success',}).then((result) => {window.location.reload();})
			}
			else
			{
...
function Hapus(id_packed){
	swal({
  title: "",
  text: "Yakin ingin menghapus data packed ini?",
  icon: "info",
  dangerMode: true,
  buttons: ["Batal", "Hapus"],
	})
	.then(willConfirm => {
	  if (willConfirm) {
	    $.post("func/func_administrator.php",{token:token, id_packed:id_packed, kode:'del_packed'},
			function(data)
			{
				if(data == 1)
				{				
...
function Editlinen(id_packed, packing_code, packing_date, packed_by, hotel_code, hotel_name, total, status, linen_status){
	$('#id_packed').val(id_packed);
...
function Lihatlinen(id_packed, packing_code, packing_date, packed_by, hotel_code, hotel_name, total, status, linen_status){
	$('#id_packed').val(id_packed);
	$('#packing_code').val(packing_code);
	$('#packing_date').val(packing_date);
	$('#packed_by').val(packed_by);
