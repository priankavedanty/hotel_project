    ...
    <div class="col-12">
      ...
      <div class="box" style="border-top:#1E58C1 4px solid; padding: 20px;">
        <div class="card-header">
          <h3 class="card-title"><b> DATA LINEN CATEGORY </b></h3>
        ...
        <div class="card-body">
          <table id="example1" class="table table-bordered table-hover">
            ...
            <tbody>
            	<?php $no2=0;
									$query = "SELECT * from tb_linen_category
									inner join tb_jabatan on tb_linen_category.id_jabatan = tb_jabatan.id_jabatan
									where id_category order by id_category ASC";
									...
									while($row = mysqli_fetch_assoc($result)) 
							{
								...
								?><tr><?php
									...
									?><td align="center"> 
											...
											<button class="btn btn-warning" title="Edit Data" onclick="Editcategory('<?=$row['id_category']?>', '<?=$row['linen_code']?>', '<?=$row['linen_name']?>', '<?=$row['description']?>')" ><i class="fa fa-pencil" <?php if($public_id_jabatan != 3 || $public_id_jabatan != 5){ print "disabled";}?>></i></button>

											<button class="btn btn-success" title="Lihat Data" onclick="Lihatcategory('<?=$row['id_category']?>', '<?=$row['linen_code']?>', '<?=$row['linen_name']?>', '<?=$row['description']?>')" ><i class="fa fa-eye" <?php if($public_id_jabatan != 3 || $public_id_jabatan != 5){ print "disabled";}?>></i></button>
									
									<button class="btn btn-danger" title="Hapus Data" onclick="Hapus('<?=$row['id_category']?>')"><i class="fa fa-trash" <?php if($public_id_jabatan != 3){ print "disabled";}?>></i></button>
...
$('#btn_save').on('click', function (e) {
	...
	if(id_category == '')
	{
		if(linen_code == "" || linen_name == "" || description == "" )
		{
			swal({icon: 'error',title: 'Oops...',text: "Mohon isi semua data!",})
		}
		else
		{	
			$.post("func/func_administrator.php",{token:token, id_category:'', linen_code:linen_code, linen_name:linen_name, description:description, kode:'linen_category'},
			function(data)
			{
				if(data == 1)
				{				
					swal({text: "Data linen category berhasil disimpan!", icon: 'success',}).then((result) => {window.location.reload();})
				}
				else
				{
	...
	{
			$.post("func/func_administrator.php",{token:token, id_category:id_category, linen_code:linen_code, linen_name:linen_name, description:description, kode:'linen_category'},
		function(data){
			if(data == 1)
			{				
				swal({text: "Data linen category berhasil diperbaharui!",icon: 'success',}).then((result) => {window.location.reload();})
			}
			else
			{
...
function Hapus(id_category){
	swal({
  title: "",
  text: "Yakin ingin menghapus data linen category ini?",
  icon: "info",
  dangerMode: true,
  buttons: ["Batal", "Hapus"],
	})
	.then(willConfirm => {
	  if (willConfirm) {
	    $.post("func/func_administrator.php",{token:token, id_category:id_category, kode:'del_category'},
			function(data)
			{
				if(data == 1)
				{				
					swal({text: "Data linen category berhasil dihapus!",icon: 'success',}).then((result) => {window.location.reload();})
				}
				else
				{
					swal({icon: 'error',title: 'Oops...',text: data,})
...
function Editcategory(id_category, linen_code, linen_name, description){
...
function Lihatcategory(id_category, linen_code, linen_name, description){
	$('#id_category').val(id_category);
	$('#linen_code').val(linen_code);
	$('#linen_name').val(linen_name);
	$('#description').val(description);
